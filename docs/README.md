...
- [Prompt 节点](./nodes/prompt.md) — 文本输入源节点（执行时会调用 LLM）
- [Text 节点](./nodes/text.md) — 文本生成节点
- [Image 节点](./nodes/image.md) — 图像生成节点（支持图生图）
- [Video 节点](./nodes/video.md) — 视频生成节点（异步轮询）
- [Upload 节点](./nodes/upload.md) — 本地图片上传节点

### 工作流引擎
- [工作流执行](./workflow.md) — 拓扑排序、循环检测、级联失败、视频轮询、REST 调用层

### AI Provider
- [Provider 架构](./providers.md) — ModelProvider 接口、AgnesAdapter 实现、模型注册表、Runner 内部 REST 函数

### 状态管理
- [Store 设计](./stores.md) — canvasStore / settingsStore / taskStore 三个 Zustand store

### UI 组件
- [组件文档](./components.md) — Sidebar、TaskManager、SettingsDialog、PropertiesPanel、ApiKeyBanner
...
- 每次代码变更必须同步更新对应文档
- 新增功能需在 README.md 索引中添加链接
- 文档中的代码片段应与源码保持一致
- 架构变更需更新 architecture.md 的依赖关系图
